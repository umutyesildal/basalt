...
const METHOD: { title: string; summary: string; body: string }[] = [
  {
    title: "One token, one vault",
    summary: "One share token represents vault assets.",
  ...
  },
  {
    ...
    summary: "Mint and redeem settle in one on-chain transaction.",
  ...
  },
  {
    ...
    summary: "Targets and fees stay fixed; market moves can shift holdings.",
  ...
  {
    title: "Fee mechanics",
    summary: "Entry and exit are one-time share fees; management is ongoing dilution.",
...
const RISKS: { title: string; summary: string; body: string }[] = [
  {
    title: "Smart contract risk",
    summary: "Unaudited contracts can affect vault assets.",
  ...
  },
  {
    ...
    summary: "Programs are upgradeable; one wallet currently controls authority.",
  ...
  },
  {
    ...
    summary: "Token prices can diverge from their equity references.",
  ...
  },
  {
    ...
    summary: "A multiplier can change displayed values; raw balances remain the transfer basis.",
  ...
  },
  {
    ...
    summary: "Zap legs are sequential; a partial failure can leave swaps settled.",
  ...
  },
  {
    ...
    summary: "Fixed targets can drift as constituent prices move.",
  ...
  },
  {
    ...
    summary: "History starts at creation; no backtested or simulated returns.",
...
function MethodCard({ item }: { item: (typeof METHOD)[number] }) {
  return (
    <details className="rounded-xl border border-border bg-card p-4">
      <summary className="cursor-pointer font-medium text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        ...
        <span className="mt-1 block text-sm font-normal leading-5 text-muted-foreground">{item.summary}</span>
      </summary>
...
function RiskCard({ item }: { item: (typeof RISKS)[number] }) {
  return (
    <details className="rounded-xl border border-border bg-card p-4">
      <summary className="cursor-pointer font-medium text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        <span className="block">{item.title}</span>
    ...
    <div className="space-y-8">
      <section aria-label="Method" className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 sm:auto-rows-fr">
          {METHOD.map((item) => <MethodCard key={item.title} item={item} />)}
        </div>
      </section>
      ...
      <section aria-label="Risks" className="space-y-4">
        ...
        />
        <div className="grid gap-3 sm:grid-cols-2 sm:auto-rows-fr">
          {RISKS.map((item) => <RiskCard key={item.title} item={item} />)}
        </div>
