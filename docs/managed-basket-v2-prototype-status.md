...
- `/managed` remains an English, wallet-free explanation using visibly simulated data. Its **Try with a wallet** link appears only when the app is configured for loopback localnet and opens `/managed/lab`, a separate localnet-only page. The lab reads confirmed program accounts and vault balances, connects through the existing Phantom/Solflare adapter, and can sign create, mint, redeem, manager proposal, guardian approval/cancellation, and eligible pair fill instructions. Transactions are simulated before wallet approval.
