...
- Tests at amendment time: 178 Rust + 373 backend TS (+1 root legacy). The historical edition2024 SBF build blocker was resolved on 2026-09-19: all three artifacts build offline with the current version-3 lockfile. CPI execution paths still require a functional localnet E2E; canonical deploy keypairs are intentionally absent, so the disposable governance rehearsal proves loader authority mechanics only. See `plan.md` §8 and `docs/local-governance-rehearsal-2026-09-19.md`. This supersedes none of the normative constraints; §11 P0/P1 evidence is recorded in the `plan.md` §6 gate table.
