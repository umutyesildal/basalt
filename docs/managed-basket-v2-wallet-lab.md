...
- Automated checks: `npx vitest run app/tests/managed-chain.test.ts --no-cache`, `npm --prefix app run typecheck`, `npm --prefix app run build`, `npx tsx --tsconfig app/tsconfig.json scripts/managed-v2-client-smoke.ts`, `npx tsx --tsconfig app/tsconfig.json scripts/managed-v2-fixture-create-smoke.ts` with a running local validator. The fixture smoke covers create, propose, guardian approval, and notice-period redeem with the same local-only sample endpoint used by the page.
