...
> **Current handoff (2026-09-25):** Work in the canonical `main` checkout at `/Users/umutyesildal/orca/workspaces/createyouretf/createyouretf`. Managed Basket V2 was merged and pushed to `main` at `3eeb7be`; `codex/managed-baskets-v2` is historical implementation provenance, not the integration target. Read `handoff.md` for the preserved uncommitted governance/deployment-verifier files and the latest test evidence. Read `docs/managed-basket-v2-prototype-status.md` for implemented V2 behavior, `docs/managed-basket-v2-wallet-lab.md` for the localnet workflow, and `docs/managed-basket-v2-plan.md` for the target architecture. The V0 invariants below still govern **immutable V0**; Managed V2 is a separate opt-in program with a fixed asset pair, shared vault, delayed bounded fill, and permissionless pro-rata redemption. V2 is localnet-only; `/managed` is simulated, and `/managed/lab` is a loopback development UI. Do not claim a public V2 deployment, creator fee revenue, live xStocks, or verified extension-wallet signing. The 2026-09-18/19 counts and state below are dated snapshots, not the current V2 status.
...
> Create UX decision (2026-09-23): Public `/create` is a wallet-free, English **concept preview** for hackathon evaluation: choose a template or assets, set a 100% mix, pick an illustrative dollar amount, review, and share a validated `/preview?d=...` link. It does not buy tokens or deploy a basket. The existing **Choose → Set up → Start → Review** devnet transaction UI lives at `/create/onchain`; fees default to zero, owned constituent-token deposits have no prefilled USD amount, legal acknowledgments remain required at Review, and wallet/balance/signing checks occur at Deploy. Genesis mints 1,000,000 raw units at six decimals, displayed as **one basket share**. Earlier six-step and wallet-gated descriptions in dated sections below are historical. Name/thesis JSON is hashed locally but is not yet published as retrievable metadata in the on-chain flow; the UI discloses this limitation.
...
**NEVER call it:** `registered ETF`, `fund`, `guaranteed return`, `safe investment`, `financial advice`, `we manage your money`. The public concept label is **Stock Baskets**, explained as shareable mixes of stocks and ETFs; it does not assert that preview assets were tokenized or acquired. For deployed products use precise terms such as `strategy basket`, `index basket`, `onchain equity basket`, or `xStocks-backed strategy token` only when the backing claim is substantiated. (`LEGAL_REVIEW_REQUIRED` remains a release gate — see §12.)
...
| **Tests** | Rust `cargo test` 208 tests, backend `vitest` 596 tests | Verified total 804 tests passing on 2026-09-19 |
...
├── programs/
│   ├── whitelist/src/lib.rs    # init_config, add_mint, pause/unpause, transfer_authority (27 tests)
│   ├── basket_factory/src/lib.rs # init_factory, create_basket (45 tests, validates 2-20, sum 10k, caps, atomic seed, genesis 1M)
│   └── basket/src/lib.rs       # math + mint_in_kind/redeem_in_kind/accrue_management_fee (136 tests)
├── backend/
...
Basket immutable — no update ix. V0 fee split is protocol-wide (90% creator / 10% treasury); `FactoryConfig.creator_fee_split_bps` remains only for legacy ABI compatibility and is pinned to 9000, plus caps `entry 300, exit 100, mgmt 300`.
...
* `init_factory(treasury:Pubkey, creator_split:u16)` — legacy split argument; only 9000 is accepted in V0
...
* `accrue_management_fee()` — exact numerator remainder carry: each interval uses then-current supply and `fee = (supply*bps*elapsed + previous_remainder)/(10000*31536000)`; fee shares join supply and later intervals compound slightly. Split with the fixed protocol rule, emit `FeeAccrued`, update `last_fee_accrual_ts` (`programs/basket/src/lib.rs:131`). Zero supply or zero bps checkpoints clear the remainder. Permissionless.
...
**Mgmt streaming:** `fee = (then_current_supply * bps * elapsed + previous_remainder) / (10000*31536000)` (`backend/src/workers/feeMath.ts:10` + `basket/src/lib.rs:46`). Fee-share mints increase supply for later intervals, so the nominal annual rate is not a fixed charge against initial supply. Example `supply 10M, 200bps, 30d → 16,438` (`docs/basalt-v0-spec.md:405`). Split `creator = floor(fee*9000/10000), treasury=fee-creator`; all split dust goes to treasury.
...
* **P1:** pro-rata `V*burn/S` floor; genesis `1M` fixed vs inflation attack (factory `seed>0` + weight check); rounding dust favors remaining holders; `transfer_checked` with `decimals` from whitelist; `// RAW ONLY`; ATA substitution `owner==user` + `mint==expected` + `getAssociatedTokenAddress`; PDA seeds `#[account(seeds=[...],bump)]`; fee `≤ cap` + canonical `creator=floor(fee*9000/10000)` + `treasury=fee-creator` on every path
...
## 13. Testing — Verified snapshot (804 tests passing)

**Rust `cargo test` 208 tests** (`cargo test -p basket` 136 + `basket_factory` 45 + `whitelist` 27):
...
**TS `vitest run` 596 backend tests across 18 files** (representative suites below):

* `backend/tests/basalt.test.ts:1` 41 tests (fixed fee policy, exact management remainder/compounding, NAV, drift, weight mismatch, holdings scaled)
...
* `zap-balance-delta.test.ts` 15 tests (existing-balance exclusion, exact BigInt deltas, min-out, ordered mint identity, and partial-leg recovery)
* `protocol-policy.test.ts` 3 tests + `script-builders.test.ts` 2 tests (client fee-split conservation and legacy `init_factory` wire/source compatibility)
* `deployment-manifest-v2.test.ts` 5 tests + `governance-rehearsal.test.ts` 15 tests (honest evidence defaults, unsafe threshold/RPC-claim rejection, loopback-only authority transfer, executable/ProgramData stability, and rollback guardrails)

Total **804 tests passing** (`cargo test: 208 + backend Vitest: 596`) on 2026-09-19. See `docs/current-state-2026-09-18.md` for dated evidence. BAS-002 host-level extension-policy and raw-delta tests, BAS-003 Zap delta tests, BAS-004 checked-arithmetic boundary/property tests, BAS-005 fee-policy/client-builder invariants, and BAS-006 manifest/rehearsal guardrails are included; instruction-level extension/adversarial-hook coverage remains open under BAS-016.
...
cargo test                          # Rust 208
cargo test -p basket --lib          # 136 basket tests
npx --prefix backend vitest run --reporter=verbose   # TS 596
...
*Generated for agents by superstack (solana.new) + Basalt architect. Current verification: 2026-09-19 — **208 Rust + 596 backend tests passing**. Official mainnet xStocks remain unsupported by the interim extension-free V0 boundary; see `docs/bas-002-token2022-extension-policy.md` and `docs/current-state-2026-09-18.md`.*
...
| File | Lines | Purpose |
|------|-------|---------|
| `programs/whitelist/src/lib.rs:1` | 269 | `WhitelistConfig` + `WhitelistedMint` structs, 5 ix + 27 tests |
| `programs/basket_factory/src/lib.rs:1` | 308 | `FactoryConfig` + `Basket` alias, 2 ix + 45 tests |
| `programs/basket/src/lib.rs:1` | 700+ | `Basket` + `math` module + 3 ix + 136 tests |
...
| `backend/tests/*.ts` | 800+ | 596 tests across 18 files; representative suites are listed in §13 |
...
# 3. Run tests (current verified baseline: 208 Rust + 596 backend)
cargo test
...
* Rust `cargo test --workspace`: 208 passed (136 basket, 45 factory, 27 whitelist).
* Backend Vitest: 596 passed across 18 files.
...
* BAS-004 checked arithmetic is locally complete: economic conversions/multiplication and bps validation are checked, max-u64/property tests pass, and factory/whitelist counter-size paths fail safely. Instruction-level arithmetic and adversarial-hook coverage remains part of BAS-016.
* Current devnet constituents are project-issued mocks, not economically backed official xStocks.
* Zap-in now uses verified post-minus-pre raw deltas; mock devnet has no Jupiter routes for local constituent mints.
* Preserve unrelated working-tree changes. Never reset, clean, or overwrite user work as part of routine edits.

*End of AGENTS.md — current verification: 2026-09-19. See the dated state document for evidence and deployment caveats.*
